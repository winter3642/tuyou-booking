...
**W2 搜索与订单**（本周重头戏）：① 产品搜索筛选接口（关键词/分类/目的地/价格区间/排序，**故意不加索引**作 W3 慢 SQL 基线）；② 购物车；③ 订单下单链路（DB 版扣库存 `UPDATE ... WHERE stock >= ?` 兜底）；④ Redis Lua 原子扣减防超卖；⑤ 缓存三防（穿透/击穿/雪崩）。每步先交付代码 + 目的说明 + 验收清单。
